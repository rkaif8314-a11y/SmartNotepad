...
const inputStyle: React.CSSProperties = {
  display: 'block', width: '100%', minWidth: '100%', maxWidth: '100%', height: 48, minHeight: 48,
  flex: '1 1 100%', alignSelf: 'stretch', boxSizing: 'border-box',
  padding: '0 44px 0 42px', border: '1px solid var(--border)', borderRadius: 11,
  background: 'var(--input)', color: 'var(--foreground)', lineHeight: '20px', outline: 'none'
};
...
export default function AuthScreen() {
  ...
  return <main className="min-h-screen grid lg:grid-cols-2 bg-background text-foreground">
    ...
    <section className="flex items-center justify-center p-6 sm:p-10"><div className="w-full max-w-md min-w-0"><div className="lg:hidden flex items-center gap-2 mb-10"><AppLogo size={34}/><span className="font-bold text-lg">Smart<span className="text-primary">Notepad</span></span></div><h2 className="text-2xl font-bold tracking-tight">{mode==='login'?'Welcome back':mode==='signup'?'Create your account':'Reset your password'}</h2><p className="mt-1 text-sm text-muted-foreground">{mode==='login'?'Sign in to your cloud workspace.':mode==='signup'?'Your notes will sync securely across sessions.':'We will send a secure reset link to your email.'}</p>
      ...
      {mode==='login'&&<div className="mt-8 rounded-xl border border-primary/20 bg-primary/5 p-4 flex gap-3"><Sparkles size={17} className="text-primary shrink-0 mt-0.5"/><div className="text-xs text-muted-foreground leading-5"><p className="font-semibold text-foreground mb-0.5">Classic preview</p><p>Use your own Supabase account to sign in. No demo credentials or fake login are used.</p></div></div>}
    </div></section>
  </main>;
}

function Field({icon:Icon,label,error,children}:{icon:React.ElementType;label:string;error?:string;children:React.ReactNode}) { return <div className="w-full min-w-0"><label className="block text-sm font-medium mb-1.5">{label}</label><div className="relative w-full min-w-0" style={{width:'100%',minWidth:'100%'}}><Icon size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground z-10 pointer-events-none"/>{children}</div>{error&&<p className="text-xs text-red-500 mt-1">{error}</p>}</div>; }
