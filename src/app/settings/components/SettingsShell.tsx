...
export interface AppSettings {
  ...
  notifications: boolean; name: string; email: string;
}
const DEFAULT_SETTINGS: AppSettings = {
  ...
  showWordCount: true, compactMode: false, notifications: true, name: 'Maya Chen', email: 'maya.chen@smartnotepad.io',
};
const SETTINGS_KEY = 'smartnotepad_settings';
function loadSettings(): AppSettings {
  if (typeof window === 'undefined') return DEFAULT_SETTINGS;
  try { const raw = localStorage.getItem(SETTINGS_KEY); return raw ? { ...DEFAULT_SETTINGS, ...JSON.parse(raw) } : DEFAULT_SETTINGS; } catch { return DEFAULT_SETTINGS; }
}
...
export default function SettingsShell() {
  const router = useRouter(); const [active, setActive] = useState<SettingsCategory>('appearance');
  const [settings, setSettings] = useState<AppSettings>(DEFAULT_SETTINGS); const [isDirty, setIsDirty] = useState(false); const [isSaving, setIsSaving] = useState(false);
  useEffect(() => setSettings(loadSettings()), []);
  useEffect(() => { const root = document.documentElement; if (settings.theme === 'dark') root.classList.add('dark'); else if (settings.theme === 'light') root.classList.remove('dark'); else window.matchMedia('(prefers-color-scheme: dark)').matches ? root.classList.add('dark') : root.classList.remove('dark'); }, [settings.theme]);
