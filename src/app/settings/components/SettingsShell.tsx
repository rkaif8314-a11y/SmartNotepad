...
const NAV_ITEMS: { key: SettingsCategory; label: string; icon: React.ElementType; description: string }[] = [
  { key: 'appearance', label: 'Appearance', icon: Palette, description: 'Theme, colors, display' }, { key: 'editor', label: 'Editor', icon: Type, description: 'Font, autosave, formatting' },
  { key: 'search', label: 'Web Search', icon: Search, description: 'Search provider, results' }, { key: 'account', label: 'Account', icon: User, description: 'Profile, security, logout' },
  { key: 'shortcuts', label: 'Shortcuts', icon: Keyboard, description: 'Keyboard shortcuts reference' },
];

export default function SettingsShell() {
  const router = useRouter(); const [active, setActive] = useState<SettingsCategory>('appearance'); const [settings, setSettings] = useState<AppSettings>(DEFAULT_SETTINGS); const [isDirty, setIsDirty] = useState(false); const [isSaving, setIsSaving] = useState(false);
  useEffect(() => setSettings(loadSettings()), []);
  useEffect(() => { const root=document.documentElement; if(settings.theme==='dark') root.classList.add('dark'); else if(settings.theme==='light') root.classList.remove('dark'); else window.matchMedia('(prefers-color-scheme: dark)').matches ? root.classList.add('dark') : root.classList.remove('dark'); root.style.setProperty('--primary', settings.accentColor); }, [settings.theme, settings.accentColor]);
